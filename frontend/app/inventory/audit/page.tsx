...
export default function AuditPage() {
    ...
    if (!audit) {
        return (
            <div className="max-w-2xl mx-auto space-y-8 text-white p-4">
                <header className="space-y-2">
                    <h1 className="text-3xl font-black italic tracking-tighter">MODO AUDITORÍA 📝</h1>
                    <p className="text-slate-400">Selecciona un almacén para comenzar el conteo físico o continuar una sesión activa.</p>
                </header>

                <div className="p-8 rounded-3xl bg-slate-800/40 border border-slate-700 backdrop-blur-xl space-y-6">
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest text-emerald-400">Almacén a Auditar</label>
                        <select 
                            className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-emerald-500 outline-none appearance-none"
                    ...
                    <button 
                        ...
                        className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:opacity-30 disabled:hover:bg-emerald-500 text-slate-900 font-bold py-4 rounded-xl transition-all shadow-lg shadow-emerald-500/20 uppercase tracking-widest text-sm"
                    >
                        Comenzar Nueva Auditoría
                    </button>
                    
                    <div className="pt-4 border-t border-slate-700/50">
                        <Link href="/inventory" className="text-slate-400 hover:text-white text-sm transition-colors flex items-center justify-center gap-2">
        ...
        <div className="space-y-6 text-white pb-20">
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="space-y-1">
                    <h1 className="text-2xl font-black">AUDITORÍA: {audit.warehouse?.name}</h1>
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-slate-400">Ref: #{audit.id} · {new Date(audit.created_at).toLocaleDateString()}</span>
                    </div>
                </div>
                {audit.status === 'IN_PROGRESS' && (
                    <div className="flex gap-3">
                        <button onClick={handleSaveProgress} disabled={saving} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-sm font-semibold transition-colors">
                            Guardar Avance
                        </button>
                        <button onClick={handleFinish} disabled={saving} className="px-6 py-2 bg-emerald-500 hover:bg-emerald-400 text-slate-900 rounded-lg text-sm font-bold transition-all shadow-lg shadow-emerald-500/20">
                            Finalizar Auditoría
                        </button>
                    </div>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700">
                    <div className="text-xs text-slate-400 uppercase font-bold mb-1 italic">Total Ítems</div>
                    <div className="text-2xl font-black">{audit.items.length}</div>
                </div>
                <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700">
                    <div className="text-xs text-slate-400 uppercase font-bold mb-1 italic">Contados</div>
                    <div className="text-2xl font-black text-emerald-400">{audit.items.filter((it: any) => it.counted_stock !== null).length}</div>
                </div>
                <div className="p-4 rounded-2xl bg-slate-800/40 border border-slate-700">
                    <div className="text-xs text-slate-400 uppercase font-bold mb-1 italic">Discrepancias</div>
                    <div className={`text-2xl font-black ${discrepancies > 0 ? "text-red-500" : "text-slate-400"}`}>{discrepancies}</div>
                </div>
            </div>

            <div className="rounded-3xl border border-slate-700/50 bg-slate-800/20 overflow-hidden">
                <table className="w-full text-left border-collapse">
                    <thead className="bg-slate-800/60 text-[10px] uppercase font-bold text-slate-400 tracking-widest">
                        <tr>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        {audit.status === 'IN_PROGRESS' ? (
                                            <input 
                                                type="number" 
                                                placeholder="0"
                                                className={`w-20 bg-slate-900 border ${hasDiff ? "border-red-500" : "border-slate-700"} rounded px-2 py-1 text-center text-sm focus:ring-1 focus:ring-emerald-500 outline-none`}
