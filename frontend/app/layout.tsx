    ...
    <div className="flex w-full min-h-screen">
      ...
      <nav className="w-64 bg-slate-800/50 backdrop-blur-md border-r border-slate-700 p-6 flex flex-col gap-6 flex-shrink-0 sticky top-0 h-screen">
        <div className="text-2xl font-bold tracking-tight text-emerald-400">
          ALMACEN <span className="text-slate-100">1.0</span>
        </div>

        {/* Context Switcher */}
        <div className="space-y-2">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Inventario Activo</label>
          <div className="flex flex-col gap-1">
            <button
              onClick={() => handleContextChange("tuberia")}
              className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-all border ${context === "tuberia" ? "bg-emerald-600/20 border-emerald-500/50 text-emerald-400" : "bg-slate-900/40 border-slate-700 text-slate-400 hover:border-slate-500"}`}
            >
              🛠️ Tubería y Acc.
            </button>
            <button
              onClick={() => handleContextChange("refacciones")}
              className={`text-left px-3 py-2 rounded-lg text-sm font-bold transition-all border ${context === "refacciones" ? "bg-blue-600/20 border-blue-500/50 text-blue-400" : "bg-slate-900/40 border-slate-700 text-slate-400 hover:border-slate-500"}`}
            >
              ⚙️ Refacciones
            </button>
          </div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-700">
          ...
          <div className="text-[10px] text-slate-500 mt-4 text-center">Proair S.A. de C.V. - 2026</div>
        </div>
      </nav>
