...
new_names = ["TIJUANA", "HERMOSILLO", "QUERETARO"]

def update_warehouses():
    for db_path in dbs:
        print(f"\n--- Updating {db_path} ---")
        if not os.path.exists(db_path):
            print("File not found index.")
            continue
        ...
        try:
            ...
            for i, name in enumerate(new_names):
                if i < len(existing):
                    ...
                    print(f"Renaming ID {w_id}: {old_name} -> {name}")
                    cursor.execute("UPDATE warehouses SET name = ? WHERE id = ?", (name, w_id))
                else:
